...
let cache = api.register("cache", {
    ...
    getAll: function*(session, id, filename, type, cachePath) {
        ...
        return yield glob(path.join(cachePath, id + "_*." + extension));
    },
    get: function*(session, id, filename, format, cachePath) {
        ...
        const exists = yield fs.existsAsync(format.filepath);

        if (!exists) {
            if (!(yield fs.existsAsync(filename))) {
                let error = new Error("Could not find " + filename);
                error.stack = false;
                throw error;
            }
